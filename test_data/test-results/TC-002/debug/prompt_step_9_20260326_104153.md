...
| $queue-loading | queue |
| $queue-empty | queue |
| $queue-pending-count | queue |
| $queue-table | queue |
| $queue-row | queue |
| $queue-action-approve | queue |
| $queue-approve-toast | queue |
| $queue-cell-txid | queue |
| $queue-cell-side | queue |
| $queue-cell-ticker | queue |
| $queue-cell-quantity | queue |
| $queue-cell-price | queue |
| $queue-cell-total | queue |
| $queue-th-txid | queue |
| $queue-th-side | queue |
| $queue-th-ticker | queue |
| $queue-th-quantity | queue |
| $queue-th-price | queue |
| $queue-th-total | queue |
...
**Planner notes:** This sequence logs in as maker, creates two pending trades, logs out, logs in as checker, and then approves both trades. The 'CreateAndMatchTrade' macro was not used because the test explicitly requires trades to be created by one user and approved by another after a user switch.
...
Usage: `{ "action": "CreateAndMatchTrade", "locator": "", "test_input": "sector=<value>,ticker=<value>,accountType=<value>,quantity=<value>,timeInForce=<value>", "output": "" }`

**Expanded steps (what the engine executes internally):**

| # | action | locator | test_input | output |
|---|--------|---------|------------|--------|
| 1 | CALL | action_CreateBuyTrade | {"sector": "{{sector}}", "ticker": "{{ticker}}", "accountType": "{{accountType}}", "quantity": "{{quantity}}", "timeInForce": "{{timeInForce}}"} |  |
| 2 | CALL | action_ApproveTrade |  |  |
| 3 | CALL | action_CreateSellTrade | {"sector": "{{sector}}", "ticker": "{{ticker}}", "accountType": "{{accountType}}", "quantity": "{{quantity}}", "timeInForce": "{{timeInForce}}"} |  |
| 4 | CALL | action_ApproveTrade |  |  |

**End state**: Last step is `CALL` on `action_ApproveTrade`.
...
Translate step **9**: "Verify both trades are matched  [ExpectedOutput: Both trades show Matched status]"

## Rules

1. Return ONLY a JSON array of step objects: `[{"tcStep": N, "action": "...", "locator": "...", "test_input": "...", "output": ""}]`.
2. Every object in the array MUST have `"tcStep": 9`.
